    ...
    while (getline(infile, line)) {
        ...
        if (splittedLine[0] == "a") {
            ...
            if(splittedLine.size() == 5) {
                arcs[addedArcs] = make_pair(tailId, Arc(headId, stoi(splittedLine[3]), stoi(splittedLine[4]), 1, 2*addedArcs));
            }
            else {
                arcs[addedArcs] = make_pair(tailId, Arc(headId, stoi(splittedLine[3]), stoi(splittedLine[4]), stoi(splittedLine[5]), 2 * addedArcs));
            }
            ++addedArcs;
        }
    }
